...
describe("GovProj", function () {
  ...
  describe("registerDeclinedTender", () => {
    it("should add a declined tender due to cost", async () => {
      await contract.deployed();

      await contract.registerProposal("test1", "test1 Descr", 1000, 1000);

      await contract.registerTender(0, "tender1Name", 1100, 950, false);

      const numDeclinedTenders = await contract.getNumberOfDeclinedTenders();
      
      expect(numDeclinedTenders.toNumber()).to.equal(1);
    });
  });

  describe("registerDeclinedTender", () => {
    it("should add a declined tender due to time", async () => {
      await contract.deployed();

      await contract.registerProposal("test1", "test1 Descr", 1000, 1000);

      await contract.registerTender(0, "tender2Name", 950, 1100, false);
  ...
  describe("registerDeclinedTender", () => {
    it("should add a declined tender due to cost", async () => {
      ...
      await contract.registerTender(0, "tender3Name", 950, 950, true);
  ...
  describe("registerSuccessfullTender", () => {
    it("should add a successfull tender", async () => {
      await contract.deployed();
      ...
      const numSuccessTenders = await contract.getNumberOfSuccessfullTenders();
