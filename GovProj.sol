...
contract GovProj{
	...
	function addDeclinedTender(uint _proposalID, bytes32 _companyName, uint _tenderFee, uint _tenderTime, bool _declaration, string memory _reason) internal {
			...
			declinedTenders[numDTenders] = Tender(numDTenders, _proposalID, _companyName, _tenderFee, _tenderTime, 0, _declaration, _reason);
	...
	function addSuccessfullTender(uint _proposalID, bytes32 _companyName, uint _tenderFee, uint _tenderTime, bool _declaration) internal {
			...
			successfullTenders[numSTenders] = Tender(numDTenders, _proposalID, _companyName, _tenderFee, _tenderTime, 0, _declaration, "success");
	...
	*/
	...
	function getWinningTenderTime(uint _proposalID) public view returns (uint tenderTime) {
		uint tenderID = getWinningTenderId(_proposalID);
		tenderTime = successfullTenders[tenderID].timeTaken;	}
